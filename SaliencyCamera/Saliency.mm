...
const char* Saliency::SAL_TYPE_DES[SAL_TYPE_NUM] = {"RC_", "HC_", "FT_", "LC_", "SR_"};
 ...
 * @abstract    get Single Area SaliencyMap
 * @param       saliencyMap: input saliencyMap(multi area SaliencyMap)
 * @return      single Area Saliency Map
 */

Mat Saliency::getSingleAreaSaliencyMap(const cv::Mat &saliencyMap){

    Mat outimg = saliencyMap;
    
    Mat label(saliencyMap.size(), CV_16SC1);
    
    
    LabelingBS labeling;
    labeling.Exec(saliencyMap.data, (short *)label.data, saliencyMap.cols, saliencyMap.rows, false, 0);
    ...
    for(int i=0;i<labeling.GetNumOfRegions();i++){
        ...
        if(maxArea < ri->GetNumOfPixels()){
            maxArea = ri->GetNumOfPixels();
            maxAreaNumber = i;
        }
    ...
    ri = labeling.GetResultRegionInfo(maxAreaNumber);
    cout<<maxAreaNumber<<": "<<ri->GetNumOfPixels()<<endl;
    
    Mat outImg = saliencyMap;
    compare(label, maxAreaNumber+1, outImg, CV_CMP_EQ);
...
/**
 * @function    getMatSaliencyMapMat
 * @abstract    get Saliency Map.
 * @param       srcImage: input UIImage
                resizeRate: resizeRate of srcImage. (complexity is decreased.)
 * @return      Multi Area Saliency Map
 */

Mat Saliency::getSaliencyMapMat(const UIImage *srcImage, float resizeRate){
    ...
    /* resize srcImageMat */
    Mat resizedSrcImageMat = srcImageMat;
    resize(srcImageMat, resizedSrcImageMat, cv::Size() , resizeRate, resizeRate);
    Mat tempTwiceMat;
	
	resizedSrcImageMat.convertTo(resizedSrcImageMat, CV_32FC3, 1.0/255);
	saliencyMapMat = GetRC(resizedSrcImageMat) * 255;
...
Mat Saliency::runMozicFilter(const cv::Mat &img3f, const Mat saliencyMap, float resizeRate){
    ...
    /* initialize quantize flag */
    BOOL **quantizeFlag;
    quantizeFlag = new BOOL *[numSquareY];
    for(int i=0;i<numSquareY;i++){
    
        quantizeFlag[i] = new BOOL [numSquareX];
    }
    for(int y=0;y<numSquareY;y++){
    
        for(int x=0;x<numSquareX;x++){
        
            quantizeFlag[y][x] = false;
        }
    }
    ...
    for(int y = 0; y < outImageMat.rows; y++){
		for(int x = 0; x < outImageMat.cols; x++){
                resizedX = x*resizeRate; resizedY = y*resizeRate;
                
                if (saliencyMap.data[resizedY * saliencyMap.cols + resizedX] == 255U) {
...
Mat Saliency::runChaosFilter(const cv::Mat &img3f, const cv::Mat saliencyMap, float resizeRate){
    ...
    int rgbFlag = 0; // 0:R 1:G 2:B
    
    unsigned char grayColor;
    
    /* chaos filter around saliency map */
    for(int y=0;y<numSquareY-1;y++){
        for(int x=0;x<numSquareX-1;x++){
            
            grayColor =  outImageMat.data[ (y*squareSize  * outImageMat.cols + x*squareSize ) * outImageMat.channels() ] * 0.3 +  outImageMat.data[ (y*squareSize  * outImageMat.cols + x*squareSize ) * outImageMat.channels() + 1 ] * 0.59 +  outImageMat.data[ (y*squareSize  * outImageMat.cols + x*squareSize ) * outImageMat.channels() + 2 ] * 0.11;
...
//                            if(c == rgbFlag){
//                            
//                                outImageMat.data[ ((y*squareSize + Y) * outImageMat.cols + (x*squareSize + X)) * outImageMat.channels() + c ] = randomColor;
//                            }
                            outImageMat.data[ ((y*squareSize + Y) * outImageMat.cols + (x*squareSize + X)) * outImageMat.channels() + c ] = grayColor;
...
Mat Saliency::runSinFilter(const cv::Mat &img3f, const cv::Mat saliencyMap, float resizeRate){
    ...
    for(int y = 0; y < outImageMat.rows; y++){
		for(int x = 0; x < outImageMat.cols; x++){
            ...
            if (saliencyMap.data[resizedY * saliencyMap.cols + resizedX] != 255U) {
                ...
                sinX = sin(2*PI / outImageMat.rows) * outImageMat.cols;
                int deltaX = x + sinX;
                if(deltaX >= outImageMat.cols){
                
                    deltaX -= outImageMat.cols;
                }
                for(int c=0;c<outImageMat.channels();c++){
                    
                    
                    outImageMat.data[ (y * outImageMat.cols + x) * outImageMat.channels() + c ] = outImageMat.data[ (y * outImageMat.cols + deltaX) * outImageMat.channels() + c ];
                }
...
Mat Saliency::GetFT(const Mat &img3f)
{
	...
	GaussianBlur(img3f, tImg, cv::Size(3, 3), 0);
...
Mat Saliency::GetSR(const Mat &img3f)
{
	...
	GaussianBlur(img1f[0], img1f[0], cv::Size(3, 3), 0);
